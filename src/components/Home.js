  ...
  return (
    <main>
      <section className="hero container">
        <div>
          <motion.h1 initial={{x:-30, opacity:0}} animate={{x:0, opacity:1}} transition={{duration:0.6}} style={{fontSize:36,fontWeight:800}}>{site.title}</motion.h1>
          <motion.p initial={{opacity:0}} whileInView={{opacity:1}} transition={{delay:0.2}} style={{marginTop:12}}>{site.intro}</motion.p>
          <div style={{marginTop:18}}>
            <a href="/projects" className="btn" style={{marginRight:8}}>View Projects</a>
